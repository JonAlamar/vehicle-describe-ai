...
def handler(event, context):
    ...
    stock_number = event["queryStringParameters"].get("stock")
    options = event["queryStringParameters"].get("options")
    mode = event["path"].split("/")[-1]  # 'lookup' or 'generate'
    ...
    if mode == "generate" and options:
        ...
        drivetrain = row["drivetrain"]
        show_drivetrain = drivetrain.lower() in ["awd", "4wd"]
...
Transmission: {row["transmission"]}
{f"Drivetrain: {drivetrain}" if show_drivetrain else ""}
...
"""
        ...
        result = json.loads(response_model["body"].read().decode())
